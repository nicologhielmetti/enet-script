  ...
  group "hls4ml-scan" {{
    ...
    task "synth_scan" {{
      ...
      config {{
        ...
        privileged = true
        args = ["/bin/bash", "/local/enet-scan/explore-enet.sh {reuse_factor} {n_filters} {clock_period} {quantization} {precision} {input_data} {output_predictions} > /local/output.txt"]
      }}
      resources {{
        memory = 8000
      }}
